...
| 模式 | 匹配规则                                       | 脱敏规则 | 误脱敏风险                    |
|------|--------------------------------------------|---------|--------------------------|
| `phone` | 11位数字，第1位=1，第2位∈[3-9] (且第第一位数字前一个字符不为英文字母) | `PHONE_MASK` | **低** — 前缀限制严格           |
| `idcard` | 18位数字 或 17位数字+X/x  (且第第一位数字前一个字符不为英文字母)    | `IDCARD_MASK` | **低** — 任何18位数字均匹配       |
| `bankcard` | 16-19位连续数字   (且第第一位数字前一个字符不为英文字母)          | `ACCOUNT_MASK` | **低** — 范围最广，16-19位数字均匹配 |

- 已在 KV 匹配中处理的区段不会重复处理
- 不匹配时间戳（14位日期时间）、普通短ID（<16位）等其他数字序列、英文字母+数字的序列
